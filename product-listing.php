<?php 
	...
	if($_REQUEST['company_id']) {
		$SQL="SELECT * FROM `product`, `company`, `type` WHERE product_type_id = type_id AND product_company_id = company_id AND company_id = ".$_REQUEST['company_id'];
	}
	if($_REQUEST['type_id']) {
		$SQL="SELECT * FROM `product`, `company`, `type` WHERE product_type_id = type_id AND product_company_id = company_id AND type_id = ".$_REQUEST['type_id'];
	}
...
jQuery(document).ready(function() {
	...
	<div id="content_sec">
		...
		<div class="contact">
			<h4 class="heading colr">All Fruits and Vegitables</h4>
			<?php
			if($_REQUEST['msg']) { 
			?>
